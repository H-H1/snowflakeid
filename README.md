...
| 方案               | 时间戳位 | 时间精度  | 机器ID位        | 序列号位      | 时间跨度 | 吞吐(单实例) | 吞吐(分片池) |
| ------------------ | -------- | --------- | --------------- | ------------- | -------- | ------------ | ------------ |
| 本项目             | 40位     | 毫秒(1ms) | 12位(4096节点)  | 11位(2048/ms) | ~34年    | ~89万/s      | ~976万/s     |
| 本项目2            | 43位     | 毫秒(1ms) | 12位(4096节点)  | 8位(256/ms)   | ~278年   | ~12万/s      | ~102万/s     |
| 本项目3            | 42位     | 毫秒(1ms) | 12位(4096节点)  | 9位(512/ms)   | ~139年   | ~25万/s      | ~170万/s     |
| bwmarrin/snowflake | 41位     | 毫秒(1ms) | 10位(1024节点)  | 12位(4096/ms) | ~69年    | ~400万/s     | 无分片       |
| sony/sonyflake     | 39位     | 10毫秒    | 16位(65536节点) | 8位(256/10ms) | ~174年   | ~2.5万/s     | 无分片       |
| Twitter原版        | 41位     | 毫秒(1ms) | 10位(1024节点)  | 12位(4096/ms) | ~69年    | —            | —            |

三个版本机器ID均为 12位（4096节点），总位数均为 63位。序列号每少 1 位，每ms吞吐减半，时间戳多 1 位，时间跨度翻倍。

## 

## ID 结构

一个 64 位有符号整数，最高位固定为 0（保证正数），有效 63 位按如下方式分配：
...
```
2⁴⁰ - 1 = 1,099,511,627,775 ms
÷ 1000 ÷ 60 ÷ 60 ÷ 24 ÷ 365 ≈ 34.8 年（从 2024-01-01 起，到约 2058 年）
```
...
```go
shard = idx % size   // 调用方传入 goroutine 编号，路由到对应分片
